...
def add_enemy_to_queue():
    """隨機選擇一個敵人加入到隊列中"""
    enemy_type = random.choice(["cacti", "ptera", "d"])  # 隨機選擇仙人掌或翼龍
...
def create_enemies():
    ...
    if len(enemies_queue) > 0 and enemies_delay == 0:
        ...
        if enemy_type == "cacti":
            active_enemies.append(Cacti(bg_x - 100, LIMIT_LOW, [img_cacti],
                                        10))
        elif enemy_type == "ptera":
            active_enemies.append(
                Ptera(bg_x - 100, PTERA_LIMIT_LOW, img_ptera, 10))

    for enemy in active_enemies:
        enemy.move()
        gg = is_hit(ds_center_x, ds_center_y, enemy.center_x, enemy.center_y,
...
img_d = ("Screen Shot 2023-12-02 at 6.41.09 PM.png")
bg_x = img.get_width()
bg_y = img.get_height()
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            if event.key == K_UP and ds_y <= LIMIT_LOW:  # 判斷恐龍是否在地上
